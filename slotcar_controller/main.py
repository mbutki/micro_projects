...
# === Setup Pins ===
# Encoders
encoder_green_pin_a = machine.Pin(2, machine.Pin.IN, machine.Pin.PULL_UP)
encoder_green_pin_b = machine.Pin(3, machine.Pin.IN, machine.Pin.PULL_UP)
encoder_red_pin_a = machine.Pin(4, machine.Pin.IN, machine.Pin.PULL_UP)
encoder_red_pin_b = machine.Pin(5, machine.Pin.IN, machine.Pin.PULL_UP)
encoder_white_pin_a = machine.Pin(6, machine.Pin.IN, machine.Pin.PULL_UP)
encoder_white_pin_b = machine.Pin(7, machine.Pin.IN, machine.Pin.PULL_UP)
...
def white_callback(val, delta):
    global white_delta
    white_delta = delta
    print("🟢 White encoder changed, delta:", delta)

# === Setup Encoders ===
Encoder(encoder_red_pin_a, encoder_red_pin_b, callback=red_callback, div=4)
Encoder(encoder_green_pin_a, encoder_green_pin_b, callback=green_callback, div=4)
Encoder(encoder_white_pin_a, encoder_white_pin_b, callback=white_callback, div=4)

def read_select():
    return pot.read_u16() / 65535

def select_triggered(select):
    global last_sent_select
    triggered = abs(last_sent_select - select) > 0.01
    if triggered:
        last_sent_select = select
    return triggered

async def main_loop():
    global last_button_state, red_delta, green_delta, white_delta, button_delta
    ...
    while True:
        ...
        select = read_select()
        select = select if select_triggered(select) else 0
        if red_delta or green_delta or white_delta or button_delta or select:
            # Prepare data
            data = {
            ...
            if ble_uart.is_connected():
                ...
                last_no_connection_check = time.ticks_ms()
                red_delta = green_delta = white_delta = button_delta = 0
        
        if not ble_uart.is_connected():
